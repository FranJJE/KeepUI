  ...
  template: `
    <div class="max-w-3xl mx-auto p-6 md:p-10 flex flex-col gap-8">
      ...
      <div>
        ...
        <code class="text-sm text-keepui-primary">&lt;keepui-card&gt;</code>
      </div>

      <section class="rounded-lg border bg-keepui-surface border-keepui-border p-6">
        <h2 class="text-xs font-semibold uppercase tracking-widest text-keepui-text-muted mt-0 mb-6">
          ...
          @for (level of elevations; track level.value) {
            <keepui-card [elevation]="level.value">
              <div class="flex items-center justify-between mb-2">
                ...
                <code class="text-xs text-keepui-text-muted font-mono"
                  >[elevation]="{{ level.value }}"</code
                >
              </div>
              <p class="mb-2 mt-0 text-sm text-keepui-text-muted leading-relaxed">
                {{ level.description }}
              </p>
              <code class="text-xs text-keepui-text-muted">{{ level.shadowClass }}</code>
            </keepui-card>
          }
        </div>
      </section>
      ...
      <section class="rounded-lg border bg-keepui-surface border-keepui-border p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <keepui-card [elevation]="1">
            <div class="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p class="m-0 text-xs text-keepui-text-muted">Actualizado hace 2h</p>
              </div>
            </div>
          ...
          <keepui-card [elevation]="2">
            <p class="text-xs uppercase tracking-widest font-semibold text-keepui-text-muted mt-0 mb-2">
              Estadística
            </p>
            <p class="text-4xl font-bold m-0 mb-1 text-keepui-text">2,480</p>
            <p class="text-sm text-keepui-success m-0">↑ 12% respecto al mes anterior</p>
            <p class="text-xs text-keepui-text-muted mt-1 mb-0">Usuarios activos</p>
          </keepui-card>

          <keepui-card [elevation]="0">
            <p class="text-xs uppercase tracking-widest font-semibold text-keepui-text-muted mt-0 mb-3">
              Sin elevación
            </p>
            <p class="text-sm text-keepui-text-muted m-0">
              Ideal como contenedor secundario dentro de otra card o sección ya elevada.
            </p>
          </keepui-card>
          ...
          <table class="w-full text-sm text-left border-collapse min-w-max">
            <thead>
              <tr class="border-b border-keepui-border">
                <th class="py-2 px-2 font-semibold text-keepui-text-muted">Input</th>
            ...
            <tbody>
              <tr>
                <td class="py-2 px-2"><code class="text-keepui-primary">[elevation]</code></td>
                <td class="py-2 px-2 font-mono text-xs text-keepui-text-muted">0 | 1 | 2 | 3</td>
                <td class="py-2 px-2 font-mono text-xs text-keepui-text-muted">1</td>
                <td class="py-2 px-2 text-xs text-keepui-text-muted">
