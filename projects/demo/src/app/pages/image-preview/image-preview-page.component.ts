  ...
  template: `
    <div class="max-w-3xl mx-auto p-6 md:p-10 flex flex-col gap-8">
      <div>
        ...
        <code class="text-sm text-keepui-primary">&lt;keepui-image-preview /&gt;</code>
      </div>
      ...
      <section class="rounded-lg border bg-keepui-surface border-keepui-border p-6">
        <h2 class="text-xs font-semibold uppercase tracking-widest text-keepui-text-muted mt-0 mb-3">
          Cómo funciona
        </h2>
        <ol class="text-sm text-keepui-text-muted m-0 pl-5 flex flex-col gap-2 leading-relaxed">
      ...
      <section class="rounded-lg border bg-keepui-surface border-keepui-border p-6">
        ...
        <p class="text-sm text-keepui-text-muted mb-4">
          Registra un provider en <code>app.config.ts</code> según la plataforma de destino:
        </p>

        <div class="flex flex-col gap-3">
          <div>
            <p class="text-xs font-semibold text-keepui-text-muted mb-1.5 mt-0">🌐 Web</p>
            <pre class="text-xs rounded-md p-4 overflow-x-auto m-0
                        bg-keepui-background border border-keepui-border"><code>import {{ '{' }} provideKeepUi {{ '}' }} from '&#64;keepui/ui';
...
{{ '}' }};</code></pre>
          ...
          <div>
            <p class="text-xs font-semibold text-keepui-text-muted mb-1.5 mt-0">📱 Capacitor</p>
            <pre class="text-xs rounded-md p-4 overflow-x-auto m-0
                        bg-keepui-background border border-keepui-border"><code>import {{ '{' }} provideKeepUiCapacitor {{ '}' }} from '&#64;keepui/capacitor';
          ...
          <table class="w-full text-sm text-left border-collapse min-w-max">
            <thead>
              <tr class="border-b border-keepui-border">
                <th class="py-2 px-2 font-semibold text-keepui-text-muted">Signal</th>
            ...
            <tbody>
              <tr class="border-b border-keepui-border">
                <td class="py-2 px-2"><code class="text-keepui-primary">imageUrl()</code></td>
                <td class="py-2 px-2 font-mono text-xs text-keepui-text-muted">string | null</td>
                <td class="py-2 px-2 text-xs text-keepui-text-muted">URL de la imagen seleccionada.</td>
              </tr>
              <tr class="border-b border-keepui-border">
                <td class="py-2 px-2"><code class="text-keepui-primary">error()</code></td>
                <td class="py-2 px-2 font-mono text-xs text-keepui-text-muted">string | null</td>
                <td class="py-2 px-2 text-xs text-keepui-text-muted">Mensaje de error si la selección falla.</td>
              </tr>
              <tr>
                <td class="py-2 px-2"><code class="text-keepui-primary">loading()</code></td>
                <td class="py-2 px-2 font-mono text-xs text-keepui-text-muted">boolean</td>
